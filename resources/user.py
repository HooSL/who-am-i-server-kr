...
class UserRegisterResource(Resource):
    def post(self) :       
        ...
        #    잘못됬으면, 클라이언트에 응답한다.
        if len( data['password'] ) < 8 or len(data['password']) > 17 :
            return {'error' : '비밀번호 길이를 확인해주세요.'}, HTTPStatus.BAD_REQUEST

        if len( data['username'] ) < 2 or len(data['username']) > 10 :
            return {'error' : '이름 길이를 확인해주세요.'}, HTTPStatus.BAD_REQUEST
...
class UserLoginResource(Resource):
    def post(self) :
        ...
        if len( record_list ) == 0 :
            return {'error' : '존재하지 않은 회원입니다.'}, HTTPStatus.UNAUTHORIZED
        ...
        return {'result' : '로그인이 되었습니다.', 'access_token' : access_token}, HTTPStatus.OK
...
class UserLogoutResource(Resource):
    ...
    def post(self):
        ...
        #그러면, jwt가 알아서 토큰이 이 겟에 있는지 확인해서 로그아웃한 유저인지 판단한다
        jti = get_jwt()['jti']
        print(jti)
